    ...
    <>
      <nav className={`fixed top-0 left-0 w-full px-6 md:px-12 py-4 flex justify-between items-center z-[40] transition-all duration-300 bg-transparent ${isDark && !isMenuOpen ? 'text-black' : 'text-white'}`}>
        ...
        <div className={`flex items-center gap-8 text-sm font-sans tracking-wide transition-opacity duration-300 ${isMenuOpen ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
          ...
          <div className="cursor-pointer md:ml-4 hover:opacity-80 transition-opacity" onClick={() => setIsMenuOpen(true)}>
      ...
      <div
        className={`fixed inset-0 bg-[#fdfa99] z-[50] transition-transform duration-700 ease-[cubic-bezier(0.76,0,0.24,1)] flex flex-col ${isMenuOpen ? 'translate-y-0' : '-translate-y-full'
          }`}
      >
        ...
        <div className="w-full px-6 pt-20  md:px-12  flex justify-center items-center relative">
          ...
          <button
            ...
            className="absolute right-6 md:right-12 top-10 text-[#2a2a2a] hover:opacity-60 transition-opacity"
